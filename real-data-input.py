...
# 기본 경계 조건 (사용자가 수정할 수 있는 초기값) - 정수형으로 변경
GLOBAL_BOUNDS = {
    ...
    def load_and_train_model():
        ...
        if model is not None:
            ...
            for var in st.session_state['ui_display_vars']:
                # 경계값 초기화 시 global_bounds도 업데이트 (정수형으로)
                min_val = int(GLOBAL_BOUNDS.get(var, (0, 300))[0]) 
                max_val = int(GLOBAL_BOUNDS.get(var, (0, 300))[1]) 
                st.session_state['global_bounds'][var] = (min_val, max_val)
                
                if var in init_row:
                    try:
                        # 초기 조건 파일의 값 사용 (정수형으로 변환)
                        st.session_state[f'input_{var}'] = int(init_row[var])
                    except (ValueError, TypeError):
                        # 값 오류 시 경계 중앙값 사용
                        st.session_state[f'input_{var}'] = int((min_val + max_val) / 2)
            ...
            for var in global_vars:
                if var not in st.session_state['ui_display_vars']:
                    # 나머지 변수 값은 정수로 변환하여 저장
                    default_init_values[var] = int(df_weld_processed.iloc[0].get(var, 0))
...
with tab1:
    ...
    if not ui_vars and st.session_state['df_init'] is not None:
        ...
        for i, var in enumerate(ui_vars):
            ...
            # 경계값은 정수형으로 가져옵니다.
            min_val, max_val = st.session_state['global_bounds'].get(var, GLOBAL_BOUNDS.get(var, (0, 300)))
            
            default_val = st.session_state.get(f'input_{var}', int((min_val + max_val) / 2))
            
            # min_val == max_val 인 경우를 위한 안전 장치
            if min_val >= max_val:
                 min_val_safe = min_val - 1
                 max_val_safe = max_val + 1
            else:
                 min_val_safe = min_val
                 max_val_safe = max_val

            with cols[i % 3]:
                # 🌟🌟🌟 step=1, format="%d"를 사용하여 정수형 슬라이더 구현 🌟🌟🌟
    ...
    if not selected_vars:
        ...
        for i, var in enumerate(selected_vars): # 선택된 변수만 반복
            ...
            current_bounds = st.session_state['global_bounds'].get(var, GLOBAL_BOUNDS.get(var, (0, 300)))
            ...
            with cols[i % 3]:
                ...
                # 2. 조건값 선택 버튼 및 Expander (정수형 슬라이더 사용)
                with st.expander("🛠️ **조건값 상/하한 설정**"):
                    
                    # 슬라이더의 전체 범위를 설정 (안전을 위해 GLOBAL_BOUNDS를 기준으로)
                    default_min_safe = GLOBAL_BOUNDS.get(var, (0, 300))[0]
                    default_max_safe = GLOBAL_BOUNDS.get(var, (0, 300))[1]
                    ...
                    def update_bounds(v, min_v, max_v):
                        # 경계값도 정수형으로 저장
                        st.session_state['global_bounds'][v] = (int(min_v), int(max_v))
                        # UI 입력창의 중앙값으로 재설정하여 즉시 반영
                        st.session_state[f'input_{v}'] = int((min_v + max_v) / 2) 
                        st.session_state['optimization_result'] = None
                        
                    # 경계 변경 확정 버튼
                    if st.button(f'**{var}** 경계 조건 적용 및 UI 반영', key=f'apply_bounds_{var}', use_container_width=True):
                        update_bounds(var, new_min, new_max)
                        st.success(f"✅ **{var}**의 조건 범위가 [{new_min:.0f}, {new_max:.0f}]로 업데이트되었고, UI 입력창에 반영되었습니다.") # 표시도 정수형으로
        ...
        for var in unselected_vars:
            ...
            current_input_val = st.session_state.get(f'input_{var}', 0)
            st.session_state['global_bounds'][var] = (int(current_input_val), int(current_input_val))
    ...
    def run_diagnosis_callback(input_vars):
        ...
        for var in global_vars:
            if var not in full_input_data:
                full_input_data[var] = st.session_state['default_init_values'].get(var, 0) 
    ...
    def run_optimization_callback(input_vars):
        ...
        for var in global_vars:
             if var in input_vars:
                 x0_dict[var] = float(input_vars[var]) # 최적화 알고리즘은 실수(float)를 사용하므로 변환
             else:
                 x0_dict[var] = float(st.session_state['default_init_values'].get(var, 0)) 
        ...
        for var in global_vars:
            if var in input_vars:
                ...
                if var in st.session_state['selected_knowhow_vars']:
                    # 노하우가 적용되는 변수는 세션 상태의 global_bounds (사용자가 설정한 범위) 사용 (float으로 변환)
                    min_val, max_val = st.session_state['global_bounds'].get(var, GLOBAL_BOUNDS.get(var, (0, 300)))
                    bounds_list.append((float(min_val), float(max_val)))
        ...
        for var in st.session_state['ui_display_vars']:
            ...
            # 🌟🌟🌟 노하우 적용 변수만 Increase/Decrease 로직 적용
